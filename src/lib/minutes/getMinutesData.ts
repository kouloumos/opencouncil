...
export async function getMinutesData(
    ...
    function buildCouncilComposition(attendance: MinutesAttendance): MinutesCouncilComposition {
        const presentIds = new Set(attendance.present.map(m => m.personId));
        ...
        if (mayorPersonId) {
            ...
            if (mayorPerson) {
                mayor = { name: formatSurnameFirst(mayorPerson.name), present: presentIds.has(mayorPersonId) };
            }
        }
        ...
        if (adminBodyId) {
            for (const person of people) {
                ...
                if (presidentRole) {
                    president = { name: formatSurnameFirst(person.name), present: presentIds.has(person.id) };
                    break;
                }
            }
        }
    ...
    const councilComposition = overallAttendance
        ? buildCouncilComposition(overallAttendance)
        : null;
