...
export async function handleTranscribeResultGeneric(
  ...
  const existingSegmentsCount = task.transcript.speakerSegments.length;
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (options?.force) {
      await deleteExistingSpeakerData(task.transcriptId, task.workspaceId, tx);
        ...
        const speakerTag = await tx.speakerTag.create({
          data: {
            label: isMatched
              ? `SPEAKER_${speakerId}`
              : nextUnknownLabel(),
            // Only connect if we verified the speaker exists
            ...(isMatched ? { speaker: { connect: { id: matchInfo.match! } } } : {})
          }
        });
 ...
 */
function getSpeakerSegmentsFromUtterances(utterances: ApiUtterance[]): SpeakerSegment[] {
