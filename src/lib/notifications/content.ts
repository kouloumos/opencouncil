...
export async function generateEmailContent(notification: NotificationData): Promise<{
    ...
    // The city's realm decides the domain and the language: a send has no
    // request host to read either from, and the notification page's own links
    // are relative, so on the wrong realm they land on the tenant-isolated city
    // route and 404.
