 ...
 *
 * The realm decides which domain a link to that city's content must use. Admin
 * alerts resolve it from here and hand it to `realmBaseUrl`, because an alert
 * knows only the city id. Everything that already loads the city — calendar
 * sync, the transcript task — reads `city.realm` instead of paying for a second
 * query.
 */
