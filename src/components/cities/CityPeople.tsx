...
import { sortPersonsByLastName, sortPeopleByPartyRank } from '@/components/utils';
...
export default function CityPeople({
    ...
    const orderedPersons = useMemo(() => {
        if (city?.peopleOrdering === 'partyRank') {
            return sortPeopleByPartyRank(allPeople, partiesWithPersons, city.peopleOrdering);
        }
        return sortPersonsByLastName(allPeople);
