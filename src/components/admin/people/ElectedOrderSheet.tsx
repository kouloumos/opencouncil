        ...
        reader.onload = (e) => {
            try {
                ...
                const importedOrder = new Map<string, number>(
                    data.members.map((m: { roleId: string }, i: number) => [m.roleId, i])
                );

                setMembers(prev => {
                    const matched = prev.filter(m => importedOrder.has(m.roleId));
                    const unmatched = prev.filter(m => !importedOrder.has(m.roleId));
                    matched.sort((a, b) => importedOrder.get(a.roleId)! - importedOrder.get(b.roleId)!);
                    return [...matched, ...unmatched];
                });

                const matchedCount = members.filter(m => importedOrder.has(m.roleId)).length;
                toast({
                    title: t('importSuccess'),
                    description: t('importSuccessDescription', { count: matchedCount, total: members.length }),
                });
            } catch {
                toast({
                    title: t('importError'),
