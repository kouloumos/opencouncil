...
/**
 * Landing (v2) analytics. All events are namespaced `landing_*` and automatically carry the
 * shared context (device, current view) set from LandingV2, so call sites stay one-liners and
 * don't have to thread device/view down through every hook and component.
 */
...
/**
 * Capture a user interaction. The first interaction of the session also emits
 * `landing_first_action` (with `action_type`), so the "first action" funnel needs no per-handler
 * plumbing.
 */
