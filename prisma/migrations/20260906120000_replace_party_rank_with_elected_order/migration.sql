...
-- current members, then the party rank inside each party. Only current council
-- roles without an elected order are written, and their positions start after
-- the highest elected order the city's council already has, so a value set by
-- hand before this migration keeps its place. A city on the default ordering
-- had no ranks, so nothing changes there.
WITH party_size AS (
...
numbered AS (
    SELECT ranked.role_id,
           ROW_NUMBER() OVER (
               ...
               ORDER BY ranked.members DESC, ranked."partyId", ranked.rank, ranked.name
